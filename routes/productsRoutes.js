...
router.get('/getSingleProduct/:productIds', (req, res) => {
  ProductControllers.getSingleProducts(req.params.productIds).then((result) =>
    res.send(result)
  );
});
...
router.post('/udpateProduct/:idProduct', auth.verify, (req, res) => {
  ...
  if (userData.isAdmin) {
    ProductControllers.udpateProduct(req.params.idProduct, req.body).then(
...
router.post('/archiveProduct/:productId', auth.verify, (req, res) => {
  const userData = {
    isAdmin: auth.decode(req.headers.authorization).isAdmin,
  };
  if (userData.isAdmin) {
    ProductControllers.archiveProduct(req.params.idProduct).then((result) =>
      res.send(result)
    );
  } else {
    res.send(false);
...
router.post('/unarchiveProduct/:idProduct', auth.verify, (req, res) => {
  ...
  if (userData.isAdmin) {
    ProductControllers.unarchiveProduct(req.params.idProduct).then((result) =>
      res.send(result)
    );
  } else {
    res.send(false);
